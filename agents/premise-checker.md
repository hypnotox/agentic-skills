...
Require an explicit premise, the consequence if it is wrong or the parent route that depends on it, and an evidence boundary. If any is missing, return `unresolved` and name the missing input; do not ask the user or infer a broader assignment.
