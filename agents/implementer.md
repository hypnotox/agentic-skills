...
Do not invent persistent memory, decision records, plan files, caches, logs, or workflow artifacts. Ordinary tool-managed build or test output is allowed when required by authorized verification and its affected paths are understood.

## Work and receipt

Implement at the semantic owner and run focused feedback plus the strongest practical assigned oracle. Do not weaken tests, fixtures, goldens, or checks to hide failure. Make local implementation choices within the settled outcome. Return material product, compatibility, safety, or architectural choices to the parent instead of deciding them silently.
