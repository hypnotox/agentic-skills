...
Adding the oracle may be the first mutation when authorized. Route a supported fix through `agentic-implementing`. Apply `agentic-code-design` when the established cause creates a structural ownership, state, invariant, contract, dependency, or refactor-boundary question. Use `agentic-brainstorming` only when the evidence creates a material choice about outcome, scope, compatibility, safety, user-visible behavior, or system direction.
