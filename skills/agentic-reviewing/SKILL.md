...
Use this skill to review existing code or prose, a design, proposed change, diff, or implementation. Independent review is especially useful for compatibility effects, cross-boundary changes, security, concurrency, data loss, migrations, destructive behavior, substantial refactoring, generated ownership, or judgment-heavy verification.
...
The parent selects reviewers, consolidates related findings, and resolves disagreements against evidence and agreed constraints. Reviewers do not delegate. Use `agentic-subagents` to choose supported model and thinking settings when delegating. Apply the relevant focuses during direct review without presenting it as a separate independent review.
